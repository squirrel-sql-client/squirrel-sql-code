...
(
   CHAR_COLUMN CHAR() ASCII(8000)
);
CREATE TABLE CLOB_TYPE_TABLE
(
   CLOB_COLUMN VARCHAR() ASCII(1980)
);
...
(
   LONGVARCHAR_COLUMN VARCHAR() ASCII(1987)
);
CREATE TABLE NUMERIC_TYPE_TABLE
(
   NUMERIC_COLUMN FIXED
);
...
(
   VARBINARY_COLUMN LONG BYTE
);
CREATE TABLE VARCHAR_TYPE_TABLE
(
   VARCHAR_COLUMN VARCHAR() ASCII(8000)
);

SELECT max(length(BINARY_COLUMN)) FROM BINARY_TYPE_TABLE;
