...
public class DateTimeFunctionsTab extends BaseDatabasePanelTab {
	...
	private interface i18n {
		String TITLE = "Date/Time Functions";
		String HINT = "Show all the date/time functions available in DBMS";
	}

	/** Logger for this class. */
	private static ILogger s_log = LoggerController.createLogger(DateTimeFunctionsTab.class);
	...
	protected IDataSet createDataSet(ISession session) throws DataSetException {
		try {
			return session.getSQLConnection().createDateTimeFunctionsDataSet(session.getMessageHandler());
