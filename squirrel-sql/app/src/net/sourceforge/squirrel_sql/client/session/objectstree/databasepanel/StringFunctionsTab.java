...
public class StringFunctionsTab extends BaseDatabasePanelTab {
	...
	private static ILogger s_log = LoggerController.createLogger(StringFunctionsTab.class);
	...
	protected IDataSet createDataSet(ISession session) throws DataSetException {
		try {
			return session.getSQLConnection().createStringFunctionsDataSet(session.getMessageHandler());
