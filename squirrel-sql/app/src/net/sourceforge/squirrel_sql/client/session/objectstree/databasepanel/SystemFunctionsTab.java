...
public class SystemFunctionsTab extends BaseDatabasePanelTab {
	/**
	...
	private static ILogger s_log = LoggerController.createLogger(SystemFunctionsTab.class);
	...
	protected IDataSet createDataSet(ISession session) throws DataSetException {
		try {
			return session.getSQLConnection().createSystemFunctionsDataSet(session.getMessageHandler());
