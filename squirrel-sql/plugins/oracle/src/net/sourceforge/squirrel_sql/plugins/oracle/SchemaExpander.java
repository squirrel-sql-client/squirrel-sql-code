...
class SchemaExpander implements INodeExpander
{
		...
		throws SQLException
	{
		...
		IDatabaseObjectInfo dbinfo = new DatabaseObjectInfo(catalogName,
											schemaName, "PACKAGE",
											IObjectTypes.PACKAGE_GRP, md);
		ObjectTreeNode child = new ObjectTreeNode(session, dbinfo);
		child.addExpander(new PackageTypeExpander(_plugin));
		childNodes.add(child);

		ObjectType objType;
		objType = new ObjectType("CONSUMER GROUP", IObjectTypes.CONSUMER_GROUP_GRP);
		childNodes.add(createObjectTypeNode(session, catalogName, schemaName,
											md, objType));

		objType = new ObjectType("FUNCTION", IObjectTypes.FUNCTION_GRP);
		childNodes.add(createObjectTypeNode(session, catalogName, schemaName,
											md, objType));

		objType = new ObjectType("INDEX", IObjectTypes.INDEX_GRP);
		childNodes.add(createObjectTypeNode(session, catalogName, schemaName,
											md, objType));

		objType = new ObjectType("LOB", IObjectTypes.LOB_GRP);
		childNodes.add(createObjectTypeNode(session, catalogName, schemaName,
											md, objType));

		objType = new ObjectType("SEQUENCE", IObjectTypes.SEQUENCE_GRP);
		childNodes.add(createObjectTypeNode(session, catalogName, schemaName,
											md, objType));

		objType = new ObjectType("TYPE", IObjectTypes.TYPE_GRP);
		childNodes.add(createObjectTypeNode(session, catalogName, schemaName,
											md, objType));
