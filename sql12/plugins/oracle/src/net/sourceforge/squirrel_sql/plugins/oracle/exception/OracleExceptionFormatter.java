 ...
 * to that error position. It does nothing else to format the exception, beyond what the default formatter
 * does.
 * 
 * @author manningr
 */
public class OracleExceptionFormatter extends SessionAdapter implements ISessionListener, ExceptionFormatter
{
	 ...
	 */
	private final static ILogger s_log = LoggerController.createLogger(OracleExceptionFormatter.class);
	...
	private static interface i18n
	{
		...
		String POSITION_LABEL = s_stringMgr.getString("OracleExceptionFormatter.positionLabel");
	}
	...
	/**
	 * The syntax error offset function to use to determine where the error occurred in a SQL statement.
	 */
	...
	/**
	 * Pass through to default formatter, detecting the error position and setting the caret position
	 * appropriately.
	 * 
	...
	private boolean isOffsetFunctionAvailable() throws SQLException
	{
		...
		for (String functionName : functionNames)
		{
			if (OFFSET_FUNCTION_NAME.equals(functionName))
			{
				if (s_log.isDebugEnabled())
				{
					s_log.debug("isOffsetFunctionAvailable: Found offset " + "function: " + OFFSET_FUNCTION_NAME);
				}
				result = true;
				break;
			}
		...
		if (s_log.isDebugEnabled())
		{
			s_log.debug("isOffsetFunctionAvailable: Couldn't locate offset " + "function: "
