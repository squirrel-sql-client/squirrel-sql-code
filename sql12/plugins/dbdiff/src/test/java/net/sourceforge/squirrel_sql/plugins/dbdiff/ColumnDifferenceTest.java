...
public class ColumnDifferenceTest extends BaseSQuirreLJUnit4TestCase
{
	...
	public void setUp() throws Exception
	{
		...
		// init mocks
		mockMetaData =
			FwTestUtil.getEasyMockSQLMetaData("Oracle", "oracle:jdbc:thin@localhost:1521:ORCL", false);

		final String[] columnNames = new String[] { "col1", "col1", "col2" };
		final Integer[] columnTypes = new Integer[] { VARCHAR, INTEGER, VARCHAR };

		final TableColumnInfo[] columns =
			FwTestUtil.getEasyMockTableColumns("doo", "foo", "table1", asList(columnNames), asList(columnTypes));

		mockVarcharCol1Table1 = columns[0];
		mockIntegerCol1Table1 = columns[1];
		mockVarcharCol2Table1 = columns[2];

		mockVarcharCol1Table2 = FwTestUtil.getEasyMockTableColumn("doo", "foo", "table2", "col1", VARCHAR);
	...
	{
		columnDifference.setColumns(mockVarcharCol2Table1Nullable, mockVarcharCol2Table1NotNullable);
		assertTrue(columnDifference.col1AllowsNull());

		columnDifference.setColumns(mockVarcharCol2Table1NotNullable, mockVarcharCol2Table1Nullable);
	...
	{
		final TableColumnInfo mockVarcharCol1Table1Copy =
			FwTestUtil.setEasyMockTableColumnInfoType(mockVarcharCol1Table1, VARCHAR);
		columnDifference.setColumns(mockVarcharCol1Table1Copy, mockVarcharCol1Table1);
