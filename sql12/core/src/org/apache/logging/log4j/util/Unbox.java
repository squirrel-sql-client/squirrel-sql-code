...
import org.apache.logging.log4j.SQuirreLsDummyEmptyLog4jStub;
...
public class Unbox {
    ...
    private final static ILogger LOGGER = SQuirreLsDummyEmptyLog4jStub.s_log;
    ...
    private static int calculateRingBufferSize(final String propertyName) {
        final String userPreferredRBSize = "<SQuirreLEmptyLog4Unused>";
...
//                String.valueOf(RINGBUFFER_MIN_SIZE));
        try {
            ...
            if (size < RINGBUFFER_MIN_SIZE) {
                ...
                LOGGER.warn("Invalid " + propertyName + " " + userPreferredRBSize + ", using minimum size " + RINGBUFFER_MIN_SIZE + ".");
            }
            return ceilingNextPowerOfTwo(size);
        } catch (final Exception ex) {
            LOGGER.warn("Invalid " + propertyName + " " + userPreferredRBSize + ", using default size " + RINGBUFFER_MIN_SIZE + ".");
