...
public class AutoCorrector
{
   ...
   public AutoCorrector(JTextComponent txtComp)
   {
      _txtComp = txtComp;
   ...
   private void onInsertUpdate(DocumentEvent e)
   {
      try
      {
         ...
         if (' ' == insertChar.charAt(0))
         {
            ...
            final String corr = new AutoCorrectProvider().getAutoCorrects().get(autoCorrCandidate);
            if(null != corr)
            {
               ...
               if(10 > _autocorrectionsCount)
               {
                  ...
                  Main.getApplication().getMessageHandler().showMessage(s_stringMgr.getString("syntax.hasBeenAutocorr", params));
