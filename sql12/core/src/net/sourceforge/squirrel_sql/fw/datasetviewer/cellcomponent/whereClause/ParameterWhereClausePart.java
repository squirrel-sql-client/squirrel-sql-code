...
public class ParameterWhereClausePart extends AbstractWhereClausePart
{
    ...
    *
    * @param column            Column, which is part of the where clause (must be not null)
    * @param parameterValue    Value, which is used within the where-clause (must not be
    *                          null)
    * @param dataTypeComponent DataTypeComponent, which is responsible for setting the
    ...
    * (non-Javadoc)
    *
    * @see net.sourceforge.squirrel_sql.fw.datasetviewer.cellcomponent.whereClause
   ...
   public void setParameter(PreparedStatement pstmt, int position)
         throws SQLException
   {
      this.dataTypeComponent.setPreparedStatementValue(pstmt, parameterValue, position);
   }
