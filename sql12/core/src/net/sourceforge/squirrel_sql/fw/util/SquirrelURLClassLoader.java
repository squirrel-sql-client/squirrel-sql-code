...
public class SquirrelURLClassLoader extends URLClassLoader
{
	...
	public SquirrelURLClassLoader(String fileName) throws IOException
	{
		this(new File(fileName).toURI().toURL());
	}

	public SquirrelURLClassLoader(URL url)
	{
		this(new URL[] { url });
	}

	public SquirrelURLClassLoader(URL[] urls)
	{
					...
					if (className != null)
					{
						...
						catch (Throwable th)
						{
							if (logger.isInfoEnabled())
							{
								...
								logger.info(s_stringMgr.getString("MyURLClassLoader.noAssignCheck", args));
							}
						}
						if (cls != null)
						{
