 ...
 * SQLFilterSheet.java
 *
 * Created on April 5, 2003, 10:12 AM
 */
...
public class SQLFilterSheet extends BaseSheet
{
   ...
   * @param objectInfo An instance of a class containing database metadata information .
   *
   */
   ...
   * @param show A boolean that determines whether the sheet is shown or hidden.
   */  
	public synchronized void setVisible(boolean show)
	{
		if (show)
		{
			if (!isVisible())
			{
				...
				pack();
        /*
         * KLUDGE: For some reason, I am not able to get the sheet to size correctly.  It always
         * displays with a size that causes the sub-panels to have their scrollbars showing.  Add
         * a bit of an increase in the size of the panel so the scrollbars are not displayed.
         */
        Dimension d = getSize();
        d.width += 5;
        d.height += 5;
   ...
   */  
	...
	/**
	 * OK button pressed. Edit data and if ok save to aliases model and then close dialog.
	 */
    ...
    try {
      ContentsTab cTab = (ContentsTab)_session.getSessionSheet()
                                              .getObjectTreePanel()
                                              .getTabbedPaneIfSelected(DatabaseObjectType.TABLE, ContentsTab.TITLE);
      if (cTab != null)
      {
  ...
  /** Create the GUI elements for the sheet */  
    ...
    try 
    {
      ...
      ResultSet rs = sqlConnection.getSQLMetaData().getColumns((ITableInfo)_objectInfo);
      while (rs.next())
      {
		...
		{
      _session.getApplication().showErrorDialog("Unable to get list of columns, " + ex);
		}
    
		final IApplication app = _session.getApplication();

    _whereClausePanel = new WhereClausePanel(columnNames, _objectInfo.getQualifiedName());
    _orderByClausePanel = new OrderByClausePanel(columnNames, _objectInfo.getQualifiedName());
		_panels.add(_whereClausePanel);
		_panels.add(_orderByClausePanel);

 		SquirrelTabbedPane tabPane = new SquirrelTabbedPane(app.getSquirrelPreferences());
		for (Iterator it = _panels.iterator(); it.hasNext();)
		{
			ISQLFilterPanel pnl = (ISQLFilterPanel) it.next();
    ...
    tabPane.addChangeListener(new ChangeListener()
    {
      ...
      {
        setButtonLabel(((SquirrelTabbedPane)event.getSource()).getSelectedIndex());
      }
    });
  ...
  /** Create a panel that contains the buttons that control the closing of the sheet.
  ...
  /** Change the text of the 'clear' button depending on which clause panel has focus.
  ...
  private void setButtonLabel(int tabSelected) 
  {
    _clearFilter.setText("Clear " + 
                         ((tabSelected == 0) ? _whereClausePanel.getTitle() : _orderByClausePanel.getTitle()));
