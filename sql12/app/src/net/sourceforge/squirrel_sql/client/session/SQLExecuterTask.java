...
public class SQLExecuterTask implements Runnable, IDataSetUpdateableTableModel
{
   ...
   private boolean processQuery(String sql, int processedStatementCount, int statementCount) throws SQLException {
      ...
      boolean gotResults = _stmt.execute(sql);
...
// There are contradictory rules in the JDBC API Doc:
...
int updateCount = -1;
      ...
      boolean loopAround = gotResults || (updateCount != -1);
      ...
      while (loopAround) {
         // User has cancelled the query execution.
          ...
          if (gotResults) {
            ResultSet res = _stmt.getResultSet();
            if (null != res) {
	...
	// _stmt.getMoreResults() will never return true anymore if you do.
         if (gotResults && false == supportsMultipleResultSets) {
            loopAround = false;
         } else {
            gotResults = _stmt.getMoreResults();
            if (! gotResults) {
               updateCount = _stmt.getUpdateCount();
            }
            loopAround = gotResults || (updateCount != -1);
         }
      }
